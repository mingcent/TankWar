...
namespace ENet
{
    public unsafe struct Peer
    {
        ...
        public bool Send_UnSequenced(byte channelID, byte[] data, int offset, int length)
        {
            if (data == null)
            {
                throw new ArgumentNullException("data");
            ...
            using (var packet = new Packet())
            {
                packet.Create(data, offset, length, PacketFlags.Unsequenced);
                ret = Send(channelID, packet);
            }
            return ret;
        }

        public bool Send_Reliable(byte channelID, byte[] data, int offset, int length)
        {
            if (data == null)
            {
                throw new ArgumentNullException("data");
            ...
            using (var packet = new Packet())
            {
                packet.Create(data, offset, length, PacketFlags.Reliable);
                ret = Send(channelID, packet);
            }
            return ret;
        }
